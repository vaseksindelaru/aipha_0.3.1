# --- aipha/data_system/api_client.py (VERSIÓN 2.1 FINAL) ---
...
class ApiClient:
    """
    Un cliente API genérico y robusto. (versión 2.1)
    Puede devolver tanto datos JSON parseados como contenido binario crudo.
    """
    def __init__(self, ...): # (El __init__ y _init_session no cambian)
    ...
    def make_request(
        ...
        method: str,
        endpoint: str,
        parse_json: bool = True,  # <-- ¡NUEVO PARÁMETRO!
        params: Optional[Dict[str, Any]] = None,
        data: Optional[Dict[str, Any]] = None,
        json_data: Optional[Dict[str, Any]] = None,
        headers: Optional[Dict[str, Any]] = None,
    ) -> Optional[Union[Any, bytes]]: # <-- ¡TIPO DE RETORNO ACTUALIZADO!
        """
        Realiza una petición HTTP a un endpoint.

        Args:
            ... (otros args no cambian) ...
            parse_json (bool): Si es True (por defecto), intenta parsear la respuesta
                               como JSON. Si es False, devuelve el contenido
                               binario crudo (`response.content`).

        Returns:
            Optional[Union[Any, bytes]]: Datos JSON, contenido binario, o None si hay error.
        """
    ...
    def close_session(self): # (Este método no cambia)
